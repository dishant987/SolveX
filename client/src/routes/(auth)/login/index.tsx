...
function Login() {
  ...
  useEffect(() => {
    if (search.error) {
      toast({
        ...
        description: decodeURIComponent(search.error),
        duration: 5000,
      });
  ...
  }, [search.error, search.success, toast, navigate]);
  ...
  const loginMutation = useApiMutation<LoginResponse, LoginFormData>({
    mutationFn: (data) =>
      publicApi.post('/auth/login', data).then((data) => data.data),
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Button type="button" variant="outline" className="w-full cursor-pointer" onClick={onGoogleLogin}>
          ...
          <Button type="button" variant="outline" className="w-full cursor-pointer" onClick={onGithubLogin}>
